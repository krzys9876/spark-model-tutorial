...
class SparkTest extends AnyFunSuite with GivenWhenThen with BeforeAndAfterAll {
  ...
  test("Calculated base for next period should equal to reference data (spark functions model)") {
    ...
    val values=df
      ...
      .collect()
    val row=values(0)
    assertResult(1)(row(0))
    assertResult(4)(row(1))
    assertResult(20.787)(row(2))
    assert(TestUtils.equalsRounded(19.112667,row(3),6))
    assertResult(5)(row(4))
    assert(TestUtils.equalsRounded(21.626,row(5),4))
    assert(TestUtils.equalsRounded(1.3413,row(6),4))
    assert(TestUtils.equalsRounded(14.9195,row(7),4))
  }
